...
def calculate_score_and_reasons(basic_data):
    ...
    growth = 0.9 if any(kw in (basic_data.get('Scope', '') or '') for kw in ['高新', '半导体', '新能源']) else 0.5
...
<body class="bg-gray-100 min-h-screen pb-12">
    <div class="max-w-2xl mx-auto px-4">
        ...
        <div class="ios-card p-6">
            <h2 class="text-xl font-semibold mb-4">客户列表 (''' + str(customers.total) + ''' 家)</h2>
            <div class="space-y-4">
'''

    for c in customers.items:
        html += '''<div class="flex justify-between items-center border-b pb-4 last:border-none">
            <div class="flex-1">
                <div class="font-medium">''' + c.company_name + '''</div>
                <div class="text-xs text-gray-500">''' + (c.uscc or '无信用代码') + '''</div>
            </div>
            <div class="text-right">
                <div class="text-3xl font-bold ''' + ('text-emerald-500' if c.score >= 80 else 'text-amber-500' if c.score >= 60 else 'text-gray-400') + '''">''' + str(c.score) + '''</div>
...
def fetch_customers():
    if request.method == 'POST':
        ...
        for item in results:
            ...
            score, grade, reasons = calculate_score_and_reasons(basic)
            cust = Customer(company_name=name, uscc=basic.get('CreditNo'), score=score, grade=grade, reasons=reasons)
            db.session.add(cust)
            new_count += 1
        db.session.commit()
        return jsonify({'status': 'success', 'msg': f'成功新增 {new_count} 家客户'})
    
    return '''<h1 class="text-2xl font-semibold text-center mt-10">🚀 自动抓客户</h1>
<form method="post" class="max-w-md mx-auto mt-8 p-6 ios-card">
    <input type="text" name="keyword" placeholder="输入关键词（如：半导体 高新）" class="w-full p-4 rounded-3xl border mb-4" required>
...
def import_customers():
    if request.method == 'POST':
        # 简化版（你的requirements中可能无pandas，先保证不崩）
        return jsonify({'status': 'success', 'msg': '导入功能已准备好，后续可扩展'})
    return '''<h1 class="text-2xl font-semibold text-center mt-10">📤 导入客户</h1>
<form method="post" enctype="multipart/form-data" class="max-w-md mx-auto mt-8 p-6 ios-card">
    ...
    <button type="submit" class="w-full bg-purple-600 text-white py-4 rounded-3xl font-medium">导入并评分</button>
</form>'''
...
def settings():
    ...
    return f'''<h1 class="text-2xl font-semibold text-center mt-10">🔑 API设置</h1>
