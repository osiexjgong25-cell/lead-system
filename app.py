...
def calculate_score_and_reasons(basic_data, news_count=0, news_titles=None):
    ...
    reasons_list = [
        ...
        f"[{datetime.now().strftime('%Y-%m-%d %H:%M')}] 新闻信号: 近90天找到 {news_count} 条相关新闻"
    ]
        ...
        <div class="max-w-2xl mx-auto px-4">
            ...
            <div class="ios-card p-6">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <a href="/customers" class="text-blue-600 text-sm font-medium flex items-center gap-1">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex justify-between items-center border-b pb-4 last:border-none">
                        <div class="flex-1 pr-4">
                            <div class="font-medium leading-tight">{c.company_name}</div>
...
def fetch_customers():
    if request.method == 'POST':
        ...
        if not qcc_key and not kd_key:
            return jsonify({'status': 'error', 'msg': '请至少设置一个企业查询API Key（企查查或开单果）'})

        new_count = 0
        for _ in range(min(pages, 10)):  # 限制最大页数防止滥用
            ...
            for item in results:
                ...
                cust = Customer(
                    company_name=name,
                    uscc=item.get('CreditNo') or basic.get('CreditNo'),
                    score=score,
                    grade=grade,
                    reasons=reasons,
    ...
    <form method="post" class="max-w-md mx-auto mt-8 p-6 ios-card">
        <input type="text" name="keyword" placeholder="输入行业关键词（如：半导体 新能源 高新）" class="w-full p-4 rounded-3xl border mb-4" required>
        <input type="number" name="pages" value="3" min="1" max="10" class="w-full p-4 rounded-3xl border mb-6" placeholder="抓取页数（建议3-5）">
        <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-3xl font-medium">开始抓取真实企业</button>
    </form>
    ''')
...
def import_customers():
    if request.method == 'POST':
        ...
        try:
            ...
            db.session.commit()
            return jsonify({'status': 'success', 'msg': f'成功导入并评分 {new_count} 家客户'})
        except Exception as e:
            return jsonify({'status': 'error', 'msg': f'导入失败: {str(e)[:80]}'})
...
def settings():
    if request.method == 'POST':
        ...
        return jsonify({'status': 'success', 'msg': 'API Key 已保存生效'})
    ...
    <div class="max-w-lg mx-auto mt-8 p-8 ios-card space-y-8">
        <form method="post">
            <div class="mb-6">
                <label class="block text-sm font-medium mb-2">企查查 Key（推荐主用）</label>
                <input type="text" name="qichacha_key" value="{get_config('qichacha_key')}" class="w-full p-4 rounded-3xl border" placeholder="请输入企查查开放平台Key">
            </div>
            <div class="mb-6">
            ...
            <div class="mb-8">
                ...
                <p class="text-xs text-gray-500 mt-1">用于抓取近3个月融资/扩张新闻，提升评分</p>
            </div>
            <button type="submit" class="w-full bg-amber-500 hover:bg-amber-600 text-white py-4 rounded-3xl font-medium">保存所有API配置</button>
        </form>
    </div>
    ''')
...
def customers_list():
    page = request.args.get('page', 1, type=int)
    customers = Customer.query.order_by(Customer.fetched_at.desc()).paginate(page=page, per_page=20, error_out=False)
    return render_template_string(f'''
    <h1 class="text-2xl font-semibold text-center mt-10">全部客户 ({customers.total} 家)</h1>
    <div class="max-w-2xl mx-auto mt-6 p-6 ios-card">
        {"".join(f"<div class='py-4 border-b last:border-none'><strong>{c.company_name}</strong> — 评分 <span class='text-emerald-600'>{c.score}</span> ({c.grade}级)</div>" for c in customers.items)}
    </div>
    ''')
